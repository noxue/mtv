...
impl WxPay {
    ...
    /// 微信支付
    async fn pay_req(&self, params: PayParams) -> Result<WxData, anyhow::Error> {
    ...
    pub async fn pay(
        ...
        let wx_data = self.pay_req(params).await?;
        Ok(wx_data)
    }
    ...
    /// 微信支付，回调解密
    pub fn decode(&self, params: &WxPayNotify) -> Result<WxNotifyData, anyhow::Error> {
...
mod test {
    ...
    fn test_jsapi() {
        ...
        rt.block_on(async {
            ...
            let res = pay.pay_req(params).await;
            debug!("res:{:#?}", res);
